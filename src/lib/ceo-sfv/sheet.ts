...
/** How far a deadline is from today, in words: "5 days overdue", "due in 6 days". */
function humanizeDue(deadlineDay: EpochDay, todayDay: EpochDay): string {
...
export async function loadShortFormVideos(): Promise<ShortFormVideos> {
  ...
  for (const r of rows) {
    if (!String(r[COL.client] ?? "").trim()) continue;
    const status = String(r[COL.status] ?? "").trim() || "No status";
    const key = status.toLowerCase();
    if (DROP_STATUSES.has(key)) continue;

    const award = String(r[COL.award] ?? "").trim().replace(/\s+20\d\d$/, "") || "Unassigned";
    let g = groups.get(award);
    if (!g) {
      g = { award, total: 0, done: 0, overdueCount: 0, worstOverdue: null, soonestUpcoming: null, perStatus: new Map() };
      groups.set(award, g);
    }
    g.total++;
    ...
    const deadline = isDone ? null : parseDeadline(String(r[COL.updates] ?? ""));
    if (deadline !== null) {
