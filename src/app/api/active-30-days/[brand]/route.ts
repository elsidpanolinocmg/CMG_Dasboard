...
import { fetchActiveWindow } from "@/lib/sources/ga4";
...
export async function GET(
  ...
  const value = await getCache().getOrLoad(
    cacheKeys.activeWindow(brand, DAYS),
    () => fetchActiveWindow(row.ga4PropertyId!, DAYS, row.ga4Filter),
    { ttlMs: ttls.ACTIVE_WINDOW, staleMs: ttls.ACTIVE_WINDOW_STALE },
  );
  return NextResponse.json({ brand, days: DAYS, value });
}
