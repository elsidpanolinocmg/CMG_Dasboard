...
export const BINDING_SPECS: Record<string, BindingSpec> = {
  ...
  ceo_short_form_videos: {
    ...
    summary: "The short-form-video status breakdown and total for the CEO Short Form Videos board.",
    usedBy: ["CEO → Short Form Videos"],
    readBy: ["src/lib/ceo-sfv/sheet.ts"],
    layout: {
      tabs: ["2026"],
      columns: [
        { column: "A", name: "Awards Name" },
        { column: "B", name: "Winner / Client Name", note: "A row counts as a video only when this is filled." },
        { column: "J", name: "Status", note: "Sent to client / Approved / Proceed by default count as done; Cancelled is excluded." },
        { column: "K", name: "Status Updates", note: "A \"deadline: 25 Sept\" note sets the video's deadline; the latest one wins." },
      ],
      notes: [
        "One row per video from row 2 down; row 1 holds the headers.",
        "Statuses are counted from the rows directly — the \"2026 summary\" tab is no longer read.",
        "Only \"deadline:\" notes are read as deadlines; dates the client owes (\"submit materials by …\") are ignored.",
        "Last updated is the workbook's Drive edit time.",
      ],
    },
  },
